...
- **Blocking Event Loop**: `from tuya_sharing import Manager` in Executor verschoben (HA 2025.12+ strict mode, Issue #8)
- **Reconfigure: Local Key vorausgefüllt**: Connection- und Full-Reconfigure zeigen jetzt den gespeicherten Local Key als Default — User sehen dass der Key vorhanden ist und überschreiben ihn nicht versehentlich
...
- **Backofen Programmnamen**: Select-Entity zeigt jetzt Klartextnamen statt roher Enum-Werte (F1 Auftaustufe, F2 Großflächengrill, F7 Ober/Unterhitze, P3 Pizza, P8 Rind, etc.)
