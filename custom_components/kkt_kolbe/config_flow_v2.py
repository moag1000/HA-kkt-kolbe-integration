...
from homeassistant.const import CONF_LOCAL_KEY
from homeassistant.helpers import selector

from .api import TuyaAPIError
from .api import TuyaAuthenticationError
...
class KKTKolbeConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input: dict[str, Any | None] | None = None) -> dict[str, Any]:
        """Handle the initial step - choose integration mode."""
        if user_input is not None:
            self.integration_mode = user_input[CONF_INTEGRATION_MODE]

            if self.integration_mode == MODE_API_DISCOVERY or self.integration_mode == MODE_HYBRID:
    ...
    async def async_step_manual_config(self, user_input: dict[str, Any | None] | None = None) -> dict[str, Any]:
    ...
    async def async_step_api_config(self, user_input: dict[str, Any | None] | None = None) -> dict[str, Any]:
    ...
    async def async_step_api_discovery(self, user_input: dict[str, Any | None] | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            for device in devices:
                ...
                if any(
                    indicator in name or indicator in model
    ...
    async def async_step_device_analysis(self, user_input: dict[str, Any | None] | None = None) -> dict[str, Any]:
    ...
    async def async_step_hybrid_manual_details(
        self, device_config, user_input: dict[str, Any | None] | None = None
    ) -> dict[str, Any]:
